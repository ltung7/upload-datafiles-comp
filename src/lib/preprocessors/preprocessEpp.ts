...
type EppResult = Record<string, string[]>;

/**
 * Recombines raw lines that were split across false newlines inside quoted strings.
 */
function normalizeEppLines(rawLines: string[]): string[] {
...
function parseEpp(lines: string[]): EppResult {
    ...
    for (const line of cleanLines) {
        ...
        if (state === 'content' && currentType) {
            const row =
                joinWithHeader && headerLine !== null
                    ? `${headerLine},${line}`
                    : line;
...
const preprocessEppFile: PreprocessFunction = async (file: File, datafiles: DataFilesType<any>, extraData: any) => {
    if (!datafiles.epp) return false;
